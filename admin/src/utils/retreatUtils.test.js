...
describe("retreatUtils", () => {
  ...
  describe("isGeneralRetreat", () => {
    test("identifies general retreats", () => {
      expect(isGeneralRetreat({ retreatType: "General Retreat" })).toBe(true);
      expect(isGeneralRetreat({ retreatType: "සාමාන්‍ය වැඩසටහන" })).toBe(true);
    });

    test("returns false for non-general retreats", () => {
      expect(isGeneralRetreat({ retreatType: "Advanced" })).toBe(false);
    });
  });
