...
export function registerPhoneNumberTextBox() {
    ElementFactory.Instance.registerElement(PHONE_NUMBER_QUESTION_TYPE, (name) => {
        return new PhoneNumberTextBoxModel(name);
    });

    ReactQuestionFactory.Instance.registerQuestion(PHONE_NUMBER_QUESTION_TYPE, (props) => {
...
export class PhoneNumberTextBox extends SurveyQuestionElementBase {
    ...
    handleValidityChange = (valid) => {
        this.question.setPropertyValue(PHONE_NUMBER_QUESTION_VALIDITY_PROPERTY, valid);
    };
